...
public class GetPageCondition extends PageCondition {
    ...
    private String sort = "";
    @JsonIgnore
    private HashMultimap<String, String> sortMap = HashMultimap.create();
     ...
     * @return
     */
    public HashMultimap<String, String> getSortMap() {
        if (!sortMap.isEmpty()
                || sort.trim().length() == 0) {
            return sortMap;
        }
    ...
    public <T> EntityWrapper<T> entityWrapper() {
        ...
        HashMultimap<String, String> sortedMap = getSortMap();
        sortedMap.keys().forEach(key -> getSortedOneValue(key).ifPresent(value ->
